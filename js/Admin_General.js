...
function SubcontenidoDashboard(tipo) {
    ...
    var url;
    if (tipo === 'sucursales') {
        url = URLSubContenido + "Files-Dashboard/File-proyectos.php";
    } else if (tipo === 'tickets') {
        url = URLSubContenido + "Files-Dashboard/File-ticket.php";
    } else if (tipo === 'miembros') {
        url = URLSubContenido + "Files-Dashboard/File-miembros.php";
    }
    ...
    return $.ajax({
        ...
        success: function (datos) {
            ...
            // Busca el botón correspondiente y añade la clase 'active'
            const clickedButton = Array.from(document.querySelectorAll('.list-button_select'))
                .find(button => button.getAttribute('onclick') && button.getAttribute('onclick').includes(tipo));
...
function CargarSubcontenidoDashboardtext(button) {
    ...
    console.log("Contenedor de ticket:", ticketContainer); // Verifica que el contenedor es el correcto.
...
// Agregar evento al botón de cancelar para cerrar el modal
document.getElementById('btn_cancelar-proyecto').addEventListener('click', function () {
    var modal = document.querySelector('.proyect');
...
function SubcontenidoConfiguracion(tipo) {
    // Definimos la URL según el contenido que se desea cargar
    var url;
    if (tipo === 'lista') {
        url = URLSubContenido + "Files-Configuracion/File-List.php";
    }
    ...
    return $.ajax({
        ...
        success: function (datos) {
            // Si la respuesta es exitosa, cargamos el contenido
            $('#SubcontenidoConfiguracion').html(datos);
        },
        error: function (xhr, status, error) {
            // Mostramos más información sobre el error
            console.error("Error AJAX:", status, error);
            console.error("Respuesta del servidor:", xhr.responseText);
            $('#SubcontenidoConfiguracion').html('<p>Hubo un error al cargar los datos.</p>');
        }
    });
}
